...
public class AnalyticsService {
    ...
    public List<Map<String, Object>> getMostProfitableCustomers() {
        ...
        return totalSpent.entrySet().stream()
                .map(entry -> {
                ...
                .sorted(Comparator.comparingDouble(c -> -(double) c.get("Net Revenue")))
                .limit(3)
    ...
    public List<Map<String, Object>> getMonthlyRetentionRate() {
        ...
        customerOrdersByMonth.forEach(row -> customersPerMonth
                .computeIfAbsent((int) row[0], k -> new HashSet<>()).add((long) row[1]));
        ...
        for (int month = 1; month <= 12; month++) {
            ...
            int returningCustomers = (int) currentMonthCustomers.stream().filter(previousMonthCustomers::contains).count();
    ...
    public Map<String, Double> getAverageOrderValueByCategory() {
        return orderItemRepository.getOrderValuePerCategory().stream()
                .collect(Collectors.toMap(
                        row -> (String) row[0],
                        row -> ((BigDecimal) row[1]).doubleValue() / ((Long) row[2])));
    }
    ...
    public Map<String, Double> getSeasonalRevenue() {
        ...
        revenueByMonth.forEach(row -> {
            ...
            seasonalRevenue.put(season, seasonalRevenue.getOrDefault(season, 0.0) + ((BigDecimal) row[1]).doubleValue());
        });
